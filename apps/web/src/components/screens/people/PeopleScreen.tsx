...
export function PeopleScreen(): React.JSX.Element {
  ...
  // `?sheet=people#headcount` — the Company tab's People card promises the
  // headcount plan, where every band carries its own slider and its own
  // ticket, without a scroll. The fragment is not a search param, so it is read
  // off the location, and only in a browser: this component also renders on the
  // server, where there is no location and nothing to scroll.
  const planRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    if (typeof window === 'undefined' || window.location.hash !== '#headcount') return;
    const frame = window.requestAnimationFrame(() => planRef.current?.scrollIntoView({ block: 'start' }));
  ...
  return (
    <>
      ...
      <div id="headcount" ref={planRef} className="scroll-mt-2">
