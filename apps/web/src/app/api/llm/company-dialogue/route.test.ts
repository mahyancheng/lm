...
describe('company dialogue proposal drafting', () => {
  it('persists a reviewable command without executing it and avoids claiming signature', async () => { const response = await POST(request()); const body = await response.json(); expect(mocks.calls).toHaveLength(1); expect(body.output.text).toContain('Nothing is queued'); expect(body.output.commands).toHaveLength(1); expect(body.receipts).toEqual([]); expect(mocks.append.mock.calls[0]![0].output.commands).toEqual(body.output.commands); });
