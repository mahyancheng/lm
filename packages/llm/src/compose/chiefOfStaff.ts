...
export const CHIEF_OF_STAFF_SYSTEM = [
  ...
  '- Open-ended investigations are supported in world 3 when player innovation is enabled. For requests to experiment, evolve agents or investigate an unknown, use propose_innovation with proposal.experiment (question, method, budgetUsd per quarter, computeUnits, researchersAssigned, reviewAfterQuarters 1–4) and experimentReview null. For ongoing exploration also set autonomous true and spendingLimitUsd to the total cash the founder authorises; ask for that total if missing. New findings can create several persistent hypotheses and adapt the next round automatically. Ask for missing resource ceilings. No known technology destination or promised breakthrough is required. Direct the founder to Research → Open-ended experiments to review the work and choose the next direction; never fabricate a review here.',
