...
import { AiLabel, Drawer, Icon, KeyValueGrid, Panel, SectionHeading, Tag, type IconName } from '@/components/ui';
...
import { hrefOfAction, sheetHref, tabPath } from '@/lib/sheets';
import {
...
const MANUAL_TICKETS: readonly { readonly href: string; readonly label: string; readonly icon: IconName; readonly blurb: string }[] = [
  { href: sheetHref('capital'), label: 'Capital', icon: 'coins', blurb: 'Raise, borrow, buy back, list' },
  { href: sheetHref('people'), label: 'People', icon: 'people', blurb: 'Hire, cut, poach, appoint' },
  { href: sheetHref('research'), label: 'Research', icon: 'flask', blurb: 'Budgets, programmes, publication' },
  { href: sheetHref('products'), label: 'Products', icon: 'box', blurb: 'Price, launch, retire' },
  { href: sheetHref('government'), label: 'Government', icon: 'capitol', blurb: 'Bid, decline, form a consortium' },
  { href: sheetHref('deals'), label: 'Deal Room', icon: 'handshake', blurb: 'Propose, accept, acquire' },
];

export function ChiefOfStaffScreen(): React.JSX.Element {
    ...
    <>
      ...
      <Panel title="Do it by hand" subtitle="The click path and the conversation produce the same objects" iconName="desk" iconTone="info">
        ...
        <p className="mt-2 text-[10.5px] leading-relaxed text-ink-faint">
          ...
          <span className="figure">{hrefOfAction('raise_round')}</span> and the rest.
        </p>
      </Panel>
      ...
      <Panel title="Queued this quarter" subtitle="What End Quarter will be asked to lock" iconName="stamp" iconTone={queue.length === 0 ? 'neutral' : 'brand'}>
        ...
        <Link href={tabPath('play')} className="btn tap-target mt-2.5 w-full" onClick={() => setBriefingOpen(false)}>
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="flex min-h-[46dvh] min-w-0 flex-col gap-4 lg:col-span-2">
          ...
          {/* --- the composer ------------------------------------------------
              Pinned above the phone's tab bar. The offset adds the home
              indicator's inset to the bar's own height, so the composer comes
              to rest exactly on the bar and never under it. From `lg` it is an
              ordinary card at the foot of the column. */}
          <div
