...
export function DealRoomScreen(): React.JSX.Element {
  const session = useSession();
  const view = usePlayerView();
  const company = useActiveCompany();

  const [selected, setSelected] = useState<string | null>(null);
  // `?sheet=deals&target=<companyId>` — a portfolio position sends the founder
  // here to bid, and this is the only acquisition desk in the app, so the
  // target arrives named rather than hunted for in the picker.
  const searchParams = useSearchParams();
  const target = searchParams?.get('target') ?? null;
  const [radarPick, setRadarPick] = useState<string | null>(target);
  useEffect(() => {
    if (target !== null) setRadarPick(target);
  }, [target]);
