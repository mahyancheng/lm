...
export function QuarterResolutionScreen(): React.JSX.Element {
  ...
  return (
    <>
      ...
      <div className="panel-surface flex flex-col gap-3 px-4 py-3.5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        ...
        {/* The screen's one forward move, and the way out of the sheet: Home is
            the company at a glance, which is where the next quarter starts. */}
        <Link href={committed ? HOME_ROUTE : tabPath('play')} className="btn btn-primary btn-lg press-pop w-full sm:w-auto">
          <Icon name={committed ? 'chevronRight' : 'back'} size={18} accent="current" />
          {committed ? 'Continue to next quarter' : 'Back to the desk'}
        </Link>
      </div>
