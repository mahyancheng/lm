            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              {founder.holdings.map((row) => (
                <PositionCard
                  ...
                  href={row.isOwnCompany ? sheetHref('capital') : sheetHref('exchange')}
                />
              ))}
            </div>
          )}
    ...
    <>
      ...
      >
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Link href={sheetHref('financials')} className="btn btn-ghost btn-sm tap-target gap-1.5 sm:min-h-0">
        ...
        <Panel iconName={TAB_ICON.subsidiaries} iconTone="brand" title="Subsidiaries" subtitle="Companies you bought outright, and companies you control on the register.">
          {portfolio.subsidiaries.length === 0 ? (
            <EmptyState
              ...
              action={
                <Link href={sheetHref('deals')} className="btn btn-sm tap-target sm:min-h-0">
                  Open the Deal Room
                </Link>
              }
            />
            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              {portfolio.subsidiaries.map((row) => (
                <PositionCard
                  ...
                  href={row.status === 'absorbed' ? sheetHref('company') : sheetHref('exchange')}
                  extraActions={
        ...
        <Panel iconName={TAB_ICON.stakes} iconTone="brand" title="Stakes" subtitle="Minority positions on other companies' registers. Crossing 5% makes one public.">
          {portfolio.stakes.length === 0 ? (
            <EmptyState
              ...
              action={
                <Link href={sheetHref('exchange')} className="btn btn-sm tap-target sm:min-h-0">
                  Open Markets
                </Link>
              }
            />
      ...
      {active === 'funds' ? (
        <Panel iconName={TAB_ICON.funds} iconTone="info" title="Funds" subtitle="A position in an institution rather than in a company.">
          <EmptyState
            ...
            action={
              <Link href={sheetHref('street')} className="btn btn-sm tap-target sm:min-h-0">
                Open The Street
              </Link>
            }
          />
