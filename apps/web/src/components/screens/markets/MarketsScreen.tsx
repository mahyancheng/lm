...
export function MarketsScreen(): React.JSX.Element {
  ...
  // `?sheet=exchange&item=<instrumentId>` — the Market tab's stock card names
  // its own instrument on the way in, so the drawer with the ticket in it is
  // open on arrival rather than one tap further down the tape.
  const searchParams = useSearchParams();
  const item = searchParams?.get('item') ?? null;
  useEffect(() => {
    if (item === null) return;
    if (!rows.some((row) => row.instrument.id === item)) return;
    setOpenId(item);
  }, [item, rows]);
