...
export function CompanyScreen(): React.JSX.Element {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <StatCard label="Headcount" iconName="people" value={headcount} unit="FTE" hint={`${employees.openRoles} roles open`} href={sheetHref('people')} />
        <StatCard
          label="Morale"
          iconName="gauge"
      ...
      <Panel
        ...
        actions={
          <Link href={sheetHref('portfolio')} className="btn btn-ghost btn-sm tap-target gap-1.5 sm:min-h-0">
      ...
      >
        {parent === null && subsidiaries.length === 0 ? (
          <EmptyState
            ...
            action={
              <Link className="btn tap-target gap-1.5" href={sheetHref('deals')}>
