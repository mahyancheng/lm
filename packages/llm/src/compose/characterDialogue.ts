...
export function composeCharacterPersona(context: CharacterUtteranceContext): string {
  ...
  return [
    ...
    bullets([
      ...
      'For an outright accelerator order, return acceleratorPurchaseDraft only when the dossier names this company as a current accelerator seller and the user supplied a concrete quantity. Copy its exact seller id and published unit price; do not offer discounts, capacity, delivery, or a seller absent from the dossier. The human must still review and confirm the order.',
